                ...
                <tbody>
                    @foreach($tiposMateriales as $tipo)
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <!-- Botón de Editar -->
                                <a href="{{ route('EditTipoMaterial', ['id' => $tipo->id]) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 text-sm uppercase font-bold rounded">
                                    Editar
                                </a>

                                <!-- Botón de Eliminar -->
                                <form action="{{ route('DeleteTipoMaterial', ['id' => $tipo->id]) }}" method="POST" class="inline-block ml-2">
                                    @csrf
                                    @method('DELETE')
