<x-app-layout>
    <div class="py-8">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-3xl mx-auto">
            <h1 class="text-lg font-bold mb-4">Crear Evento</h1>
          ...
          @endif
            <form action="{{ route('GuardarEvento') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <x-input type="text" name="titulo" id="titulo" class="w-full border rounded" value="{{ old('titulo') }}" required />
                </div>
                <div class="mb-4">
                    <x-label for="fecha" value="Fecha" />
                    <x-input type="date" name="fecha" id="fecha" class="w-full border rounded" value="{{ old('fecha') }}" required />
                </div>
                <div class="mb-4">
                    <x-label for="hora_inicio" value="Hora de Inicio" />
                    <x-input type="time" name="hora_inicio" id="hora_inicio" class="w-full border rounded" value="{{ old('hora_inicio') }}" required />
                </div>
                <div class="mb-4">
                    <x-label for="hora_fin" value="Hora de Finalización" />
                    <x-input type="time" name="hora_fin" id="hora_fin" class="w-full border rounded" value="{{ old('hora_fin') }}" />
                </div>
                <div class="mb-4">
                    <x-label for="descripcion" value="Descripción" />
                    <textarea name="descripcion" id="descripcion" class="w-full border rounded">{{ old('descripcion') }}</textarea>
                </div>
                <!-- Select: Tipo Evento -->
                <div class="w-full">
